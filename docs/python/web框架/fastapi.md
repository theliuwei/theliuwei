...
用户输入 BlogType 里面的任何一个值，都是有效的，get_blog_type 函数会被执行：
...
我们也可以对查询参数设置默认值，例如我们可以将 page 的默认值设定为 1，page_size 的默认值设定为 10（ `page=1, page_size=10` ）：
...
是，若在 page_size 的位置输入字符串（例如：abc），就会出现错误。
